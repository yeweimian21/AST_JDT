...
public class CDCBmiStatsDAOTest extends TestCase {
	private DAOFactory factory = TestDAOFactory.getTestInstance();
	
	private DAOFactory evilFactory = EvilDAOFactory.getEvilInstance();
	
	
	private CDCBmiStatsDAO evilDAO = new CDCBmiStatsDAO(evilFactory);
	
	
	private CDCBmiStatsDAO cdcDAO = new CDCBmiStatsDAO(factory);
	...
	public void testStoreAndGetStats() throws DBException {
		...
		try {
			...
			fail("Exception thrown when storeStats called in CDCBmiStatsDAOTEST: line 48.");
		}
		
		try {
			cdcDAO.getCDCStats(cdcBean.getSex(), cdcBean.getAge());
		} catch (Exception e) {
			fail("Exception thrown when getCDCStats called in CDCBmiStatsDAOTEST: line 54.");
		}
