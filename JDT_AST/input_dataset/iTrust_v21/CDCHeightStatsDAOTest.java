...
public class CDCHeightStatsDAOTest  extends TestCase{
		...
		private CDCHeightStatsDAO evilDAO = new CDCHeightStatsDAO(evilFactory);
		
		
		private CDCHeightStatsDAO cdcDAO = new CDCHeightStatsDAO(factory);
		...
		public void testStoreAndGetStats() throws DBException {
			...
			try{
				...
				fail("Exception thrown when storeStats called in CDCHeightStatsDAOTEST: line 48.");
			}
			
			try{
				cdcDAO.getCDCStats(cdcBean.getSex(), cdcBean.getAge());
			}catch(Exception e){
				fail("Exception thrown when getCDCStats called in CDCHeightStatsDAOTEST: line 54.");
			}
