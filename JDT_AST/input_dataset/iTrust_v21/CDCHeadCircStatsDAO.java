 ...
 * 
 */
public class CDCHeadCircStatsDAO extends CDCStatsDAO {
	 ...
	 */
	public CDCHeadCircStatsDAO(DAOFactory factory) {
	...
	public void storeStats(CDCStatsBean statsBean) throws DBException {
		...
		try {
			...
			ps = conn.prepareStatement("INSERT INTO cdcheadcircumferencestats (sex, age, L, M, S) VALUES (?,?,?,?,?) "
	...
	public CDCStatsBean getCDCStats(int sex, float age) throws DBException {
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT * FROM cdcheadcircumferencestats WHERE sex=? AND age=?");
			ps.setInt(1, sex);
			ps.setFloat(2, age);
